...
class WeixinController extends Controller
{
	...
	public function actionOauthCRM($callback, $scope = 'snsapi_userinfo')
	{
		$_SESSION['callback_url']=$callback;
		$wechatObj = new Weixin();
		$url=$wechatObj->getOauthCRM($scope);
		Header('Location:'.$url);
		Yii::app()->end();
	}

	public function actionOauthbrow($callback, $scope = 'snsapi_userinfo')
	{
		$_SESSION['callback_url']=$callback;
		$wechatObj = new Weixin();
		$url=$wechatObj->getOauthCRM($scope);
	...
	public function actionCallbackcrm(){
		$code=isset($_GET['code'])&&$_GET['code']!='authdeny'?$_GET['code']:"";
		if(!$code){
			unset($_SESSION['weixin_info_id']);
			Header('Location:'.$_SESSION['callback_url']);
		...
		if(isset($rs['access_token'])){
			$_SESSION['access_token']=$rs['access_token'];
			$_SESSION['openid']=$rs['openid'];	
			$sql="select * from same_weixin_crm where openid='".$_SESSION['openid']."'";
			$ins=Yii::app()->db->createCommand($sql)->select()->queryRow();
			if(!isset($ins['id'])){
				$info=file_get_contents("https://api.weixin.qq.com/sns/userinfo?access_token=".$_SESSION['access_token']."&openid=".$_SESSION['openid']."&lang=zh_CN");
				$info=json_decode($info,true);
				$sql="insert into same_weixin_crm set openid='".$_SESSION['openid']."',nickname='".$info['nickname']."',sex='".$info['sex']."',province='".$info['province']."',city='".$info['city']."',country='".$info['country']."',headimgurl='".$info['headimgurl']."'";
				Yii::app()->db->createCommand($sql)->execute();
				//$command ->bindParam(":nickname",json_encode(array("name"=>$info['nickname'])), PDO::PARAM_STR);
				$_SESSION['weixin_info_id']=Yii::app()->db->lastInsertID;
			}else{
			...
			$urlAry = explode("?", $_SESSION['callback_url']);
			if(count($urlAry)>1)
				$url = $urlAry[0]."?openid=".$rs['openid']."&access_token=".$rs['access_token']."&".$urlAry[1];
			else
				$url = $urlAry[0]."?openid=".$rs['openid']."&access_token=".$rs['access_token'];
			Header('Location:'.$url);
			Yii::app()->end();
		}
	}
	...
	{
		$wechatObj = new Weixin();
		print $access_token=$wechatObj->getpublictoken();
		Yii::app()->end();
	}

	public function actionGetJsticket()
	{
		$wechatObj = new Weixin();
		print $tiket=$wechatObj->getpublicticket();
		Yii::app()->end();
	}
	...
	public function actionCallback2(){
		...
		if(isset($rs['openid'])){
			...
			$id=Yii::app()->db->createCommand($sql)->select()->queryScalar();
			if (!$id) {
				$sql="insert into same_weixin_openid set openid='".$_SESSION['openid']."'";
				Yii::app()->db->createCommand($sql)->execute();
				$_SESSION['weixin_info_id']=Yii::app()->db->lastInsertID;
			} else {
				$_SESSION['weixin_info_id']=$id;
			}
			Header('Location:'.$_SESSION['callback_url']);
			Yii::app()->end();
		}
