...
class Wmenu{
	...
	public function eventAdd($data)
	{
		...
		if ($msgtype == 'pic') {
			...
			$qurl = "https://api.weixin.qq.com/cgi-bin/material/add_material?access_token=". $accesstoken;
			$post_data = array("type" => "image", "media" => '@'. substr($data['bpicUrl'], 1));	
			$ch=curl_init();
			...
			$output = curl_exec($ch);
			curl_close($ch);
			$imageInfo = json_decode($output, true);
			$url = $imageInfo['url'];
			$content = $imageInfo['media_id'];
		}
		if($msgtype=='text'){
			$title = '';
			$description = '';
